locals {
  ...
  artifacts_bucket_name  = "s3-codepipeline-${var.app_name}-${local.env_type}"
...
module "build" {
  ...
  environment_variables                 = merge(var.environment_variables, { APPSPEC = templatefile("${path.module}/templates/appspec.json.tpl", { APP_NAME = "${var.app_name}", ENV_TYPE = "${local.env_type}", HOOKS = local.pipeline_type != "dev", HOOK_TYPE = local.deploy_hooks, PIPELINE_TYPE = local.pipeline_type }) })
...
module "code-deploy" {
  ...
  env_type                         = local.env_type
  app_name                         = var.app_name
  s3_bucket                        = "s3-codepipeline-${var.app_name}-${local.env_type}"
